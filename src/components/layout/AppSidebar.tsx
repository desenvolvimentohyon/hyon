...
export function AppSidebar() {
  ...
  const initials = (currentUser?.nome || profile?.full_name || "U").split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase();

  // ───── Collapsed = Dock mode (circular icons with tooltips) ─────
  if (collapsed) {
    return (
      <Sidebar collapsible="icon">
        <SidebarHeader className="p-2 pb-1">
          <div className="flex items-center justify-center py-2">
            <img src={logoHyon} alt="Hyon" className="h-9 w-9 object-contain" />
          </div>
        </SidebarHeader>

        <SidebarContent className="pt-1 px-1.5">
                ...
                return (
                  <SidebarMenuItem key={mod.id}>
                    <Tooltip>
                      ...
                      <TooltipContent side="right" className="bg-card border-border shadow-elevated text-xs font-medium">
                        {mod.title}
                      </TooltipContent>
                    </Tooltip>
                  </SidebarMenuItem>
        ...
        <SidebarFooter className="p-2 pt-1">
          ...
          <TooltipProvider delayDuration={100}>
            <Tooltip>
              ...
              <TooltipContent side="right" className="bg-card border-border shadow-elevated text-xs">
  ...
  return (
    <Sidebar collapsible="icon">
      ...
      <Separator className="mx-4 w-auto bg-sidebar-border/30" />
      ...
      <SidebarContent className="pt-1">
        {/* Favorites section */}
        {favoriteItems.length > 0 && !lowerSearch && (
          <SidebarGroup>
