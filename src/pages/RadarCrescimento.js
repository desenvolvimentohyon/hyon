...
import { Rocket, TrendingUp, TrendingDown, DollarSign, Users, Percent, Sparkles } from "lucide-react";
...
export default function RadarCrescimento() {
    ...
    const conversionStats = useMemo(() => {
        ...
        conversionMetrics.forEach(m => {
            if (!statusMap[m.risk_type])
                statusMap[m.risk_type] = { total: 0, converted: 0 };
            statusMap[m.risk_type].total++;
            if (m.conversion_status === 'concluido')
                statusMap[m.risk_type].converted++;
        });
    ...
    return (<div className="space-y-6">
      <PageHeader title="Radar de Crescimento" subtitle="Métricas estratégicas para crescer mais rápido" icon={Rocket} iconClassName="text-success"/>
      <ModuleNavGrid moduleId="inteligencia"/>

      <div className="grid gap-4 grid-cols-2 lg:grid-cols-3">
        {kpiCards.map(k => (<Card key={k.label} className="neon-border">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="neon-border">
          ...
          <CardContent className="h-[250px] pt-4">
            {conversionStats.length > 0 ? (<ResponsiveContainer width="100%" height="100%">
                <BarChart data={conversionStats} layout="vertical">
                  ...
                  <Tooltip cursor={{ fill: 'rgba(139, 92, 246, 0.05)' }} content={({ active, payload }) => {
                if (active && payload && payload.length) {
                    const data = payload[0].payload;
                    return (<div className="bg-card border border-primary/20 p-2 rounded shadow-lg text-[10px]">
                ...
                return null;
                  ...
                  <Bar dataKey="rate" radius={[0, 4, 4, 0]} barSize={24}>
                    {conversionStats.map((entry, index) => (<Cell key={`cell-${index}`} fill={index === 0 ? "#8B5CF6" : "#A78BFA"}/>))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>) : (<div className="flex items-center justify-center h-full text-muted-foreground text-xs italic">
