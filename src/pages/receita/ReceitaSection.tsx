...
import { fmt, fmtShort, useReceitaMetricas } from "./useReceitaMetricas";
import { ReceitaCharts } from "./ReceitaCharts";

/**
 * Bloco de conteúdo de Receita / MRR sem PageHeader nem ModuleNavGrid,
 * pensado para ser usado dentro de abas em outra página.
 */
export function ReceitaSection() {
  ...
  const activeSystemNames = useMemo(
    () => sistemaCatalogo.filter((s) => s.ativo).map((s) => s.nome),
    [sistemaCatalogo]
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {kpis.map((k) => (
          <Card
            key={k.label}
            className="domain-border-left transition-all duration-200 hover:-translate-y-0.5 shadow-card hover:shadow-card-hover"
