...
import { AlertTriangle, Plus, Users, TrendingUp, TrendingDown, Headphones, FileText, Send, ThumbsUp, Ban, DollarSign, Percent, Activity, Shield, BarChart3, PieChart as PieChartIcon, ExternalLink, RefreshCw, Download, Clock, Zap, CalendarPlus, Target, Sparkles } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { RECEITA_COLORS, getSystemColor } from "@/types/receita";
import { useParametros } from "@/contexts/ParametrosContext";
import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, Tooltip as RechartsTooltip, ResponsiveContainer, CartesianGrid, Area, AreaChart, } from "recharts";
...
function KpisSkeleton() {
    return (<div className="grid gap-4 grid-cols-2 lg:grid-cols-5">
      {Array.from({ length: 5 }).map((_, i) => (<Card key={i} className="neon-border">
...
function ChartsSkeleton() {
    return (<div className="grid gap-4 lg:grid-cols-12">
      ...
      <div className="lg:col-span-4 grid gap-4">
        {[1, 2].map(i => (<Card key={i} className="neon-border">
...
function OperationalSkeleton() {
    return (<div className="grid gap-4 lg:grid-cols-3">
      {Array.from({ length: 3 }).map((_, i) => (<Card key={i} className="neon-border">
          <CardHeader className="pb-2"><Skeleton className="h-4 w-36"/></CardHeader>
          <CardContent className="space-y-2">
            {Array.from({ length: 4 }).map((_, j) => (<Skeleton key={j} className="h-10 w-full rounded-lg"/>))}
          </CardContent>
        </Card>))}
    </div>);
}
...
function TasksSkeleton() {
    return (<Card className="neon-border">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
...
function Sparkline({ data, color, height = 32 }) {
    ...
    return (<svg width={w} height={height} className="opacity-60">
...
function ChartTooltipContent({ active, payload, label }) {
    if (!active || !payload?.length)
        return null;
    return (<div className="bg-popover border border-border rounded-lg px-3 py-2 shadow-elevated text-xs">
      <p className="text-muted-foreground mb-1">{label}</p>
      {payload.map((p, i) => (<p key={i} className="font-medium" style={{ color: p.color }}>{p.name}: {typeof p.value === "number" ? fmt(p.value) : p.value}</p>))}
    </div>);
}
// ── Indicações Card ──────────────────────────────────────────────────
    ...
    return (<Card className="neon-border">
      ...
      <CardContent>
        <div className="space-y-2">
          {referrals.map(r => (<div key={r.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-border/50 hover:bg-accent/50 transition-colors duration-150">
...
function PlanosVencendoCard() {
    ...
    if (!expiring || expiring.length === 0)
        return null;
    return (<Card className="neon-border border-warning/30">
      <CardHeader className="pb-3">
        <CardTitle className="text-sm flex items-center gap-2">
      ...
      <CardContent>
        <div className="space-y-2">
          {expiring.map((c) => (<div key={c.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-border/50 hover:bg-accent/50 cursor-pointer transition-colors duration-150" onClick={() => navigate(`/clientes?id=${c.id}`)}>
              <div className="flex-1 min-w-0">
...
function CertificadosVencendoCard() {
    ...
    return (<Card className="neon-border border-warning/30">
      ...
      <CardContent>
        <div className="space-y-2">
          {expiring.map((c) => (<div key={c.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-border/50 hover:bg-accent/50 cursor-pointer transition-colors duration-150" onClick={(e) => openWhatsApp(c, e)} title="Clique para falar no WhatsApp sobre a renovação">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{c.name}</p>
              ...
              <Badge className={`text-[10px] ${c.days_left < 0 || c.days_left <= 7
...
// ── IA Insights Card ────────────────────────────────────────────────
function IAInsightsCard({ receitaMetricas }) {
    ...
    const sendPushAlert = async (title, body, targetUserIds) => {
        try {
        ...
        const checkExpiredPlans = async () => {
            ...
            if (expiredPlans && expiredPlans.length > 0) {
                expiredPlans.forEach(plan => {
                    sendPushAlert("Plano de Recuperação Expirado", `O plano para risco de ${plan.risk_type === 'inadimplencia' ? 'Inadimplência' : 'Churn'} atingiu a data limite sem conclusão.`);
                });
    ...
    const handleGeneratePlan = async (level, riskType) => {
    ...
    const showRecoveryPlan = async (insight, riskType = "inadimplencia") => {
        setActiveAnalysis({
    ...
    if (isLoading)
        return <Card className="neon-border border-primary/20 bg-primary/5 mb-4"><CardContent className="p-4"><div className="space-y-2"><Skeleton className="h-4 w-3/4"/><Skeleton className="h-4 w-1/2"/><Skeleton className="h-4 w-2/3"/></div></CardContent></Card>;
      ...
      <Card className="neon-border border-primary/20 bg-primary/5 mb-4 shadow-glow-primary/5">
        ...
        <CardContent className="space-y-3 pb-5">
          {insights.map((insight, i) => {
            ...
            return (<div key={i} className="flex flex-col gap-2 p-2 rounded-lg hover:bg-primary/5 transition-colors border border-transparent hover:border-primary/10">
                ...
                {isRisk && (<div className="flex gap-2 ml-4">
                    <Button variant="ghost" size="sm" className="h-6 text-[9px] gap-1 text-primary hover:text-primary hover:bg-primary/10" onClick={() => sendPushAlert("Alerta de Risco Hyon", cleanInsight)}>
                      <Send className="h-2.5 w-2.5"/> Disparar Alerta
                    </Button>
                    <Button variant="ghost" size="sm" className="h-6 text-[9px] gap-1 text-purple hover:text-purple hover:bg-purple/10" onClick={() => showRecoveryPlan(cleanInsight)}>
        ...
        <AlertDialogContent className="max-w-2xl glass-premium border-primary/20">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-foreground/90 text-sm whitespace-pre-line leading-relaxed overflow-y-auto max-h-[60vh] py-4">
              {activeAnalysis?.loading ? (<div className="space-y-3">
                  <Skeleton className="h-4 w-full"/>
                  <Skeleton className="h-4 w-[90%]"/>
                  <Skeleton className="h-4 w-[95%]"/>
                  <Skeleton className="h-4 w-[85%]"/>
                </div>) : activeAnalysis?.content}
...
function RecoveryHistoryCard() {
    ...
    if (!plans && !isLoading)
        return null;
    return (<Card className="neon-border border-purple/20 bg-purple/5 mb-6">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex gap-1">
            <select className="bg-purple/10 border-none text-[10px] rounded px-1.5 h-6 text-purple outline-none cursor-pointer" value={filterRiskType} onChange={(e) => setFilterRiskType(e.target.value)}>
              <option value="all">Todos Riscos</option>
              <option value="inadimplencia">Inadimplência</option>
              <option value="churn">Churn</option>
            </select>
            <select className="bg-purple/10 border-none text-[10px] rounded px-1.5 h-6 text-purple outline-none cursor-pointer" value={filterDate} onChange={(e) => setFilterDate(e.target.value)}>
      ...
      <CardContent className="space-y-3 pb-5">
        {isLoading ? (<div className="space-y-2">
            ...
            <Skeleton className="h-12 w-full rounded-lg"/>
          </div>) : plans?.length === 0 ? (<p className="text-[10px] text-muted-foreground text-center py-4">Nenhum plano encontrado com estes filtros.</p>) : (plans?.map((plan) => (<div key={plan.id} className="p-2.5 rounded-lg border border-purple/10 bg-purple/5 space-y-2 group transition-all hover:bg-purple/10">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <select className={`text-[9px] rounded px-1 h-5 outline-none cursor-pointer border-none font-bold ${plan.conversion_status === 'concluido' ? 'bg-success/20 text-success' :
                plan.conversion_status === 'em_execucao' ? 'bg-info/20 text-info' : 'bg-muted text-muted-foreground'}`} value={plan.conversion_status || 'pendente'} onChange={(e) => handleStatusChange(plan.id, e.target.value)}>
...
function RenovacoesCard() {
    ...
    const { data: renewals } = useQuery({
        ...
        queryFn: async () => {
            ...
            return data.map((r) => ({ ...r, client_name: clientMap.get(r.client_id) || "—" }));
        },
    });
    ...
    return (<Card className="neon-border">
      ...
      <CardContent className="space-y-4">
        {hasRenewals && (<div className="space-y-2">
            {renewals.map((r) => (<div key={r.id} className="flex items-center gap-3 p-2.5 rounded-lg border border-border/50 hover:bg-accent/50 cursor-pointer transition-colors duration-150" onClick={() => {
                    if (r.proposal_public_token)
                        window.open(`/proposta/${r.proposal_public_token}`, "_blank");
                    else
                        navigate(`/clientes?id=${r.client_id}`);
                }}>
                <div className="flex-1 min-w-0">
        ...
        {hasAlerts && (<>
            <div className="pt-2 border-t">
              ...
              <div className="space-y-1.5">
                {alertLogs.map((l) => (<div key={l.id} className="flex items-center gap-2 p-2 rounded-lg border border-border/30 bg-muted/30">
...
export default function Dashboard() {
    ...
    const navigate = useNavigate();
    const [chartMode, setChartMode] = useState("mrr");
    const [evolutionScope, setEvolutionScope] = useState("realizado");
    ...
    const atrasadas = tarefas.filter(t => {
        if (!t.prazoDataHora || t.status === "concluida" || t.status === "cancelada")
            return false;
    ...
    const propostasVencendo = propostas.filter(p => {
        if (!p.dataValidade || p.statusAceite === "aceitou")
            return false;
    ...
    const statusDistribution = useMemo(() => {
        ...
        clientesReceita.forEach(c => { if (counts[c.statusCliente] !== undefined)
            counts[c.statusCliente]++; });
        return [
    ...
    const evolutionData = useMemo(() => {
        ...
        return months.map(m => {
            const items = (evolutionRaw || []).filter((t) => t.competency === m.key);
            const receita = items.filter((t) => t.type === "receber").reduce((s, t) => s + Number(t.value_final || 0), 0);
            const despesa = items.filter((t) => t.type === "pagar").reduce((s, t) => s + Number(t.value_final || 0), 0);
    ...
    const sparkChurn = useMemo(() => {
        ...
        return months.map(m => {
            const cancelados = cancels.filter((c) => (c.cancelled_at || "").slice(0, 7) === m).length;
            const baseAtivos = created.filter((c) => (c.created_at || "").slice(0, 7) <= m).length;
    ...
    return (<TooltipProvider delayDuration={200}>
      <div className="space-y-6 chart-container">
        {/* ── Header ─────────────────────────────────────────────── */}
        <PageHeader title="Dashboard" subtitle={`Bem-vindo, ${tecnicoNome}`} actions={<div className="flex items-center gap-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="sm" onClick={() => navigate("/reunioes?new=1")} className="gap-1.5" aria-label="Agendar reunião">
        ...
        {dataLoading ? <KpisSkeleton /> : (<div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-5">
          {receitaKpis.map(k => (<Card key={k.label} className="group cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-elevated border border-border/60 bg-card" onClick={() => navigate("/clientes?tab=receita")}>
              <CardContent className="p-4 lg:p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="h-7 w-7 rounded-lg flex items-center justify-center transition-transform duration-300 group-hover:scale-110" style={{ backgroundColor: `${k.color}18` }}>
                ...
                {k.variation !== null && k.variation !== undefined && (<div className="mt-2 flex items-center gap-1">
                    {(() => {
                        ...
                        return (<span className={`inline-flex items-center gap-0.5 text-[10.5px] font-semibold ${cls}`}>
        ...
        {dataLoading ? <ChartsSkeleton /> : (<div className="grid gap-4 lg:grid-cols-12">
          ...
          <Card className="lg:col-span-8 neon-border">
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-0.5 bg-muted/40 rounded-md p-0.5">
                    {["realizado", "previsto"].map(s => (<Button key={s} variant={evolutionScope === s ? "secondary" : "ghost"} size="sm" className="text-[10.5px] h-6 px-2" onClick={() => setEvolutionScope(s)}>
                        {s === "realizado" ? "Realizado" : "Previsto"}
                      </Button>))}
                  </div>
                  <div className="flex gap-1">
                    {["mrr", "custos", "margem"].map(m => (<Button key={m} variant={chartMode === m ? "secondary" : "ghost"} size="sm" className="text-[11px] h-7 px-2.5" onClick={() => setChartMode(m)}>
            ...
            <CardContent className="pb-4">
              {!evolutionHasData ? (<div className="h-[280px] flex flex-col items-center justify-center text-center text-muted-foreground gap-2">
                ...
                </div>) : (<ResponsiveContainer width="100%" height={280}>
                <AreaChart data={evolutionData}>
                  ...
                  {chartMode === "mrr" && (<Area type="monotone" dataKey="MRR" stroke={RECEITA_COLORS.receita} fill="url(#gradMrr)" strokeWidth={2} dot={false}/>)}
                  {chartMode === "custos" && (<Area type="monotone" dataKey="Custos" stroke={RECEITA_COLORS.custos} fill="url(#gradCustos)" strokeWidth={2} dot={false}/>)}
                  {chartMode === "margem" && (<Area type="monotone" dataKey="Margem" stroke={RECEITA_COLORS.margem} fill="url(#gradMargem)" strokeWidth={2} dot={false}/>)}
                </AreaChart>
              </ResponsiveContainer>)}
            </CardContent>
          </Card>
          ...
          <div className="lg:col-span-4 grid gap-4">
            ...
            <Card className="neon-border">
              ...
              <CardContent className="pb-3">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 justify-center mt-1">
                  {statusDistribution.map(s => (<div key={s.name} className="flex items-center gap-1.5 text-[11px]">
            ...
            <Card className="neon-border">
              ...
              <CardContent className="pb-3">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 justify-center mt-1">
                  {sistemasMini.map(s => (<div key={s.name} className="flex items-center gap-1.5 text-[11px]">
                      <span className="w-2 h-2 rounded-full" style={{ background: s.color }}/>
                      <span className="text-muted-foreground">{s.name}</span>
        ...
        {allLoading ? <OperationalSkeleton /> : (<div className="grid gap-4 lg:grid-cols-3">
          {/* Clientes em atraso */}
          <Card className="neon-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
            ...
            <CardContent>
              {receitaMetricas.emAtraso.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-4">Nenhum cliente em atraso ✨</p>) : (<div className="space-y-2">
                  {receitaMetricas.emAtraso.slice(0, 5).map(c => (<div key={c.id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-accent/50 transition-colors cursor-pointer" onClick={() => navigate("/clientes")}>
                  ...
                  {receitaMetricas.emAtraso.length > 5 && (<p className="text-[11px] text-muted-foreground text-center pt-1">+{receitaMetricas.emAtraso.length - 5} clientes</p>)}
          ...
          <Card className="neon-border">
            ...
            <CardContent>
              <div className="space-y-2">
                {crmSummary.map(s => (<div key={s.status} className="flex items-center gap-2 p-2 rounded-lg hover:bg-accent/50 transition-colors cursor-pointer" onClick={() => navigate("/crm")}>
                ...
                <div className="flex gap-2 pt-2 border-t border-border/50">
                  {propostasKpis.map(k => (<div key={k.label} className="flex-1 text-center p-2 rounded-lg bg-accent/30">
        ...
        {dataLoading ? <AnalyticsSkeleton /> : (<div className="grid gap-4 lg:grid-cols-2">
          {/* Top Suporte */}
          <Card className="neon-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
            ...
            <CardContent>
              {topSuporte.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-6">Sem dados de suporte</p>) : (<ResponsiveContainer width="100%" height={220}>
          ...
          <Card className="neon-border">
            ...
            <CardContent>
              {custosMini.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-6">Sem dados de custos</p>) : (<ResponsiveContainer width="100%" height={220}>
        ...
        {(receitaMetricas.alertaCritico30.length > 0 || receitaMetricas.alertaCritico7.length > 0) && (<div className="grid gap-4 lg:grid-cols-2">
            {receitaMetricas.alertaCritico30.length > 0 && (<Card className="border-destructive/40 bg-destructive/5 neon-border h-full" style={{ borderLeftColor: RECEITA_COLORS.custos }}>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm flex items-center gap-2">
                ...
                <CardContent>
                  <div className="space-y-1.5">
                    {receitaMetricas.alertaCritico30.slice(0, 3).map(c => (<div key={c.id} className="flex items-center gap-2 p-2 rounded-lg border border-destructive/20 hover:bg-destructive/5 cursor-pointer transition-colors" onClick={() => navigate("/clientes")}>
                        <span className="text-xs font-medium flex-1 truncate">{c.nome}</span>
                    ...
                    <div className="mt-3 p-2.5 rounded-lg bg-destructive/10 border border-destructive/20 relative overflow-hidden group">
                      ...
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" className="h-6 text-[9px] border-destructive/30 hover:bg-destructive/10 text-destructive w-full" onClick={() => {
            ...
            {receitaMetricas.alertaCritico7.length > 0 && (<Card className="border-warning/40 bg-warning/5 neon-border h-full" style={{ borderLeftColor: RECEITA_COLORS.churn }}>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4 text-warning"/>
                ...
                <CardContent>
                  <div className="space-y-1.5">
                    {receitaMetricas.alertaCritico7.slice(0, 3).map(c => (<div key={c.id} className="flex items-center gap-2 p-2 rounded-lg border border-warning/20 hover:bg-warning/5 cursor-pointer transition-colors" onClick={() => navigate("/clientes")}>
                        <span className="text-xs font-medium flex-1 truncate">{c.nome}</span>
                    ...
                    <div className="mt-3 p-2.5 rounded-lg bg-warning/10 border border-warning/20 relative overflow-hidden group">
                      ...
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" className="h-6 text-[9px] border-warning/30 hover:bg-warning/10 text-warning w-full" onClick={() => {
                    navigate("/dashboard");
                    toast.info("Gere o plano no card 'Resumo Inteligente' abaixo.");
                }}>
        ...
        {propostasVencendo.length > 0 && (<Card className="neon-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
            ...
            <CardContent>
              <div className="space-y-1.5">
                {propostasVencendo.map(p => (<div key={p.id} className="flex items-center gap-2 p-2 rounded-lg border border-border/50 hover:bg-accent/50 cursor-pointer transition-colors" onClick={() => navigate(`/propostas/${p.id}`)}>
        ...
        {/* Minhas Tarefas */}
        {appLoading ? <TasksSkeleton /> : (<Card className="neon-border">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2">
          ...
          <CardContent>
            {minhasTarefas.length === 0 ? (<p className="text-muted-foreground text-sm text-center py-8">Nenhuma tarefa pendente 🎉</p>) : (<div className="space-y-1.5">
                {minhasTarefas.map(t => {
                    const tipoConfig = TIPO_OPERACIONAL_CONFIG[t.tipoOperacional] || TIPO_OPERACIONAL_CONFIG.interno;
                    return (<div key={t.id} className="flex items-center gap-2 p-2.5 rounded-lg border border-border/50 hover:bg-accent/50 cursor-pointer transition-colors duration-150" onClick={() => navigate(`/tarefas/${t.id}`)}>
